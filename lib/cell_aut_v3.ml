...
let rot f x y z =
  [|
    (f.mat.(x).(y+1).(z).(2) -. f.mat.(x).(y-1).(z).(2) -. f.mat.(x).(y).(z+1).(1) +. f.mat.(x).(y).(z-1).(1)) /. f.dx;
    (f.mat.(x).(y).(z+1).(0) -. f.mat.(x).(y).(z-1).(0) -. f.mat.(x+1).(y).(z).(2) +. f.mat.(x-1).(y).(z).(2)) /. f.dx;
    (f.mat.(x+1).(y).(z).(1) -. f.mat.(x-1).(y).(z).(1) -. f.mat.(x).(y+1).(z).(0) +. f.mat.(x).(y-1).(z).(0)) /. f.dx
  |]

let update_e f x y z dt c mu =
  (c *. c *. dt) ** rot f.b x y z -- (mu *. c *. c *. dt) ** f.j.mat.(x).(y).(z) ++ f.e.mat.(x).(y).(z)

let update_b f x y z dt =
  f.b.mat.(x).(y).(z) -- dt ** rot f.e x y z

  let update f0 f1 dt c mu =
    let n = f0.size in
    for x = 1 to n - 2 do
      for y = 1 to n - 2 do
        for z = 1 to n - 2 do
          f1.e.mat.(x).(y).(z) <- update_e f0 x y z dt c mu;
          f1.b.mat.(x).(y).(z) <- update_b f0 x y z dt
        done;
      done;
    done;
